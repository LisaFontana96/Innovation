...
# here load the raster data providedby Sofia that merges landcover, building and roads
# to create the raster I used the same landcover layer and Building layer used for the irbanisation index
# the roads layer was downloaded from Microsoft roads open source
landcover <- raster('~/Merged_landcover_build_road.tif')
...
S1 <- read.csv('~/site-loc.csv') #add directory for the csv file with your roost location 
S1_lon <- S1$Lon
S1_lat <- S1$Lat

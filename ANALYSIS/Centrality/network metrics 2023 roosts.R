...
##night time metrics
net_night<-read.csv("~network_movement_night_2023_all_roosts.csv", header=T, row.names=1)
...
write.csv(data_output_night,"~/day_network_metrics.csv")

##day time metrics
net_day<-read.csv("~/network_movement_day_2023_all_roosts.csv", header=T, row.names=1)
...
#write.csv(data_output_day,"~/night_network_metrics.csv")
